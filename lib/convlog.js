...
function qipai(attr) {
  ...
  ten.unshift(...ten.splice(attr.oya, (type.sanma ? 3 : 4) - attr.oya));
  hai.unshift(...hai.splice(attr.oya, (type.sanma ? 3 : 4) - attr.oya));
...
function hule(attr, oya) {
  ...
  sc.unshift(...sc.splice(oya, (type.sanma ? 3 : 4) - oya));
  ...
  let hule = {
    l: (+attr.who + (type.sanma ? 3 : 4) - oya) % (type.sanma ? 3 : 4),
    shoupai: [
      pai(attr.hai.split(",").filter((pn) => pn != attr.machi)) +
        pai(attr.machi),
    ]
      .concat(
        (attr.m ? attr.m.split(",") : []).reverse().map((mc) => mianzi(mc))
      )
      .join(","),
    baojia:
      attr.who != attr.fromWho
        ? (+attr.fromWho + (type.sanma ? 3 : 4) - oya) % (type.sanma ? 3 : 4)
        : null,
...
function pingju(attr, oya, fulou) {
  ...
  let hai = [0, 1, 2, 3].map((i) =>
    attr[`hai${i}`]
      ? [pai(attr[`hai${i}`].split(","))].concat(fulou[i]).join(",")
      : ""
  );

  sc = [sc[1], sc[3], sc[5], sc[7]];
  sc.unshift(...sc.splice(oya, (type.sanma ? 3 : 4) - oya));
  hai.unshift(...hai.splice(oya, (type.sanma ? 3 : 4) - oya));
...
function convlog(xml, log_id) {
  ...
  for (let tag of xml.match(/<.*?>/g)) {
    ...
    if (elem == "mjloggm") {
      ...
      if (log_id) {
        if (log_id[0] == ":") paipu.title = log_id.substr(1);
        else paipu.title += `\n${log_id}`;
      }
      ...
      let l = (+attr.who + (type.sanma ? 3 : 4) - oya) % (type.sanma ? 3 : 4);
      let m = mianzi(attr.m);
    ...
    if (attr.owari) {
      ...
      let defen = [owari[0], owari[2], owari[4], owari[6]].map((x) => x * 100);
