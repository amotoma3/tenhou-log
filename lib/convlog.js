...
function parse_player(attr) {
    ...
    return [0,1,2,3].map(n=>name[n]?`${name[n]}\n(${dan[n]} R${rate[n]})`:'');
}
...
function hule(attr, oya) {
    ...
    let hule = {
        ...
        baojia:   attr.who != attr.fromWho
                        ? (+ attr.fromWho + (type.sanma ? 3 : 4) - oya) % (type.sanma ? 3 : 4) : null,
...
function convlog(xml, log_id) {
    ...
    for (let tag of xml.match(/<.*?>/g)) {
        ...
        if (elem == 'mjloggm') {
            ...
            let l = (elem[0].charCodeAt(0) - 'T'.charCodeAt(0) + (type.sanma ? 3 : 4) - oya) % (type.sanma ? 3 : 4);
            ...
            let l = (elem[0].charCodeAt(0) - 'D'.charCodeAt(0) + (type.sanma ? 3 : 4) - oya) % (type.sanma ? 3 : 4);
            let p = pai(elem.substr(1));
