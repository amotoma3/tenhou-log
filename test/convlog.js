...
suite("convlog()", () => {
  ...
  suite("UN", () => {
    let xml =
      '<UN n0="%E5%B0%B1%E6%B4%BB%E7%94%9F%40%E5%B7%9D%E6%9D%91%E8%BB%8D%E5%9B%A3" n1="%41%53%41%50%49%4E" n2="%E2%93%85%E5%A0%80%E5%86%85%E6%AD%A3%E4%BA%BA" n3="%E2%93%85%E7%9F%B3%E6%A9%8B%E4%BC%B8%E6%B4%8B" dan="20,20,0,0" rate="2325.68,2260.51,1500.00,1500.00" sx="M,F,M,F"/>';
    test("新人", () => {
  ...
  suite("INIT", () => {
    ...
    test("持ち点", () => {
      assert.deepEqual(
        last_log(convlog(xml)).qipai.defen,
  ...
  suite("T|U|V|W", () => {
    test("赤牌アリ", () => {
      let xml =
        ...
        '<TAIKYOKU oya="0"/>' +
        '<INIT seed="5,1,2,3,3,103" ten="280,290,340,290" oya="1" hai0="48,11,26,83,39,19,22,25,57,5,27,90,12" hai1="71,91,42,23,87,76,63,129,30,79,123,88,9" hai2="6,135,102,60,4,34,50,86,13,3,128,119,18" hai3="20,29,113,117,108,49,80,78,93,73,43,41,111"/>';
      assert.deepEqual(last_log(convlog(xml + "<T16/>")), {
        zimo: { l: 3, p: "m0" },
      });
      assert.deepEqual(last_log(convlog(xml + "<U52/>")), {
        zimo: { l: 0, p: "p0" },
      });
      assert.deepEqual(last_log(convlog(xml + "<V88/>")), {
        zimo: { l: 1, p: "s0" },
      });
      assert.deepEqual(last_log(convlog(xml + "<W124/>")), {
        zimo: { l: 2, p: "z5" },
      });
    });
    test("赤牌ナシ", () => {
  ...
  suite("D|E|F|G", () => {
    test("赤牌アリ", () => {
      let xml =
        ...
        '<TAIKYOKU oya="0"/>' +
        '<INIT seed="5,1,2,3,3,103" ten="280,290,340,290" oya="1" hai0="48,11,26,83,39,19,22,25,57,5,27,90,12" hai1="71,91,42,23,87,76,63,129,30,79,123,88,9" hai2="6,135,102,60,4,34,50,86,13,3,128,119,18" hai3="20,29,113,117,108,49,80,78,93,73,43,41,111"/>';
      assert.deepEqual(last_log(convlog(xml + "<D16/>")), {
        dapai: { l: 3, p: "m0" },
      });
      assert.deepEqual(last_log(convlog(xml + "<E52/>")), {
        dapai: { l: 0, p: "p0" },
      });
      assert.deepEqual(last_log(convlog(xml + "<F88/>")), {
        dapai: { l: 1, p: "s0" },
      });
      assert.deepEqual(last_log(convlog(xml + "<G124/>")), {
        dapai: { l: 2, p: "z5" },
      });
    });
    test("赤牌ナシ", () => {
    ...
    test("ツモ切り", () => {
      let xml =
        ...
        '<TAIKYOKU oya="0"/>' +
        '<INIT seed="5,1,2,3,3,103" ten="280,290,340,290" oya="1" hai0="48,11,26,83,39,19,22,25,57,5,27,90,12" hai1="71,91,42,23,87,76,63,129,30,79,123,88,9" hai2="6,135,102,60,4,34,50,86,13,3,128,119,18" hai3="20,29,113,117,108,49,80,78,93,73,43,41,111"/>';
      assert.deepEqual(last_log(convlog(xml + "<T16/><D16/>")), {
        dapai: { l: 3, p: "m0_" },
      });
      assert.deepEqual(last_log(convlog(xml + "<U52/><E52/>")), {
        dapai: { l: 0, p: "p0_" },
      });
      assert.deepEqual(last_log(convlog(xml + "<V88/><F88/>")), {
        dapai: { l: 1, p: "s0_" },
      });
      assert.deepEqual(last_log(convlog(xml + "<W124/><G124/>")), {
        dapai: { l: 2, p: "z5_" },
      });
    });
  });
  ...
  suite("N", () => {
    suite("赤牌アリ", () => {
      let xml =
        ...
        '<INIT seed="5,1,2,3,3,103" ten="280,290,340,290" oya="1" hai0="48,11,26,83,39,19,22,25,57,5,27,90,12" hai1="71,91,42,23,87,76,63,129,30,79,123,88,9" hai2="6,135,102,60,4,34,50,86,13,3,128,119,18" hai3="20,29,113,117,108,49,80,78,93,73,43,41,111"/>';
      test("チー", () => {
        assert.deepEqual(last_log(convlog(xml + '<N who="0" m="6207"/>')), {
          fulou: { l: 3, m: "m3-40" },
        });
        assert.deepEqual(last_log(convlog(xml + '<N who="0" m="31767"/>')), {
          fulou: { l: 3, m: "p40-6" },
        });
        assert.deepEqual(last_log(convlog(xml + '<N who="0" m="57735"/>')), {
          fulou: { l: 3, m: "s067-" },
        });
      });
      test("ポン", () => {
        assert.deepEqual(last_log(convlog(xml + '<N who="1" m="6697"/>')), {
          fulou: { l: 0, m: "m505+" },
        });
        assert.deepEqual(last_log(convlog(xml + '<N who="1" m="20010"/>')), {
          fulou: { l: 0, m: "p550=" },
        });
        assert.deepEqual(last_log(convlog(xml + '<N who="1" m="34315"/>')), {
          fulou: { l: 0, m: "s555-" },
        });
        assert.deepEqual(last_log(convlog(xml + '<N who="1" m="48681"/>')), {
          fulou: { l: 0, m: "z555+" },
        });
      });
      test("加槓", () => {
        assert.deepEqual(last_log(convlog(xml + '<N who="2" m="6705"/>')), {
          gang: { l: 1, m: "m505+5" },
        });
        assert.deepEqual(last_log(convlog(xml + '<N who="2" m="20018"/>')), {
          gang: { l: 1, m: "p550=5" },
        });
        assert.deepEqual(last_log(convlog(xml + '<N who="2" m="34323"/>')), {
          gang: { l: 1, m: "s555-0" },
        });
        assert.deepEqual(last_log(convlog(xml + '<N who="2" m="48689"/>')), {
          gang: { l: 1, m: "z555+5" },
        });
      });
      test("大明槓", () => {
        assert.deepEqual(last_log(convlog(xml + '<N who="3" m="4609"/>')), {
          fulou: { l: 2, m: "m5505+" },
        });
        assert.deepEqual(last_log(convlog(xml + '<N who="3" m="13314"/>')), {
          fulou: { l: 2, m: "p5550=" },
        });
        assert.deepEqual(last_log(convlog(xml + '<N who="3" m="31746"/>')), {
          fulou: { l: 2, m: "z5555=" },
        });
      });
      test("暗槓", () => {
        assert.deepEqual(last_log(convlog(xml + '<N who="3" m="22528"/>')), {
          gang: { l: 2, m: "s5550" },
        });
        assert.deepEqual(last_log(convlog(xml + '<N who="3" m="31744"/>')), {
          gang: { l: 2, m: "z5555" },
        });
      });
    });
  ...
  suite("DORA", () => {
    ...
    test("暗槓 → 即開槓", () => {
      assert.deepEqual(
        last_log(
          convlog(
            xml + '<N who="0" m="16384" />' + '<DORA hai="14" />' + "<T62/>"
          ),
          3
        ),
        [
  ...
  suite("REACH", () => {
    ...
    test("リーチ宣言", () => {
      assert.deepEqual(
        last_log(convlog(xml + '<REACH who="2" step="1"/>' + "<F85/>")),
        { dapai: { l: 1, p: "s4*" } }
      );
      assert.deepEqual(
        last_log(
          convlog(xml + "<V85>" + '<REACH who="2" step="1"/>' + "<F85/>")
        ),
        { dapai: { l: 1, p: "s4_*" } }
      );
    });
    ...
    test("門前ツモ和了", () => {
      assert.deepEqual(
        last_log(
          convlog(
            xml +
              '<AGARI ba="0,1" hai="9,14,16,18,20,24,52,55,77,81,86,90,94,98" machi="86" ten="20,18000,2" yaku="1,1,0,1,7,1,8,1,54,2,53,0" doraHai="61" doraHaiUra="121" who="3" fromWho="3" sc="47,-60,233,-60,397,-60,313,190" />'
          )
        ),
        {
          hule: {
    ...
    test("副露ロン和了", () => {
      assert.deepEqual(
        last_log(
          convlog(
            xml +
              '<AGARI ba="0,0" hai="59,62,67,80,86,88,97,99" m="39465,1641" machi="86" ten="30,5800,0" yaku="8,1,52,1,54,1" doraHai="52" who="1" fromWho="2" sc="210,0,230,58,330,-58,230,0" />'
          )
        ),
        {
          hule: {
    ...
    test("役満", () => {
      assert.deepEqual(
        last_log(
          convlog(
            xml +
              '<AGARI ba="1,0" hai="82,84,90,121,122,124,125,127" m="51818,49707" machi="121" ten="40,48000,5" yakuman="39" doraHai="59" who="1" fromWho="3" sc="114,0,364,483,224,0,298,-483" />'
          )
        ),
        {
          hule: {
    ...
    test("ダブル役満", () => {
      assert.deepEqual(
        last_log(
          convlog(
            xml +
              '<AGARI ba="1,0" hai="112,113,114,116,119,120,121,122" m="48715,41993" machi="120" ten="40,96000,5" yakuman="50,42" doraHai="115" who="2" fromWho="3" sc="244,0,94,0,308,963,354,-963" />'
          )
        ),
        {
          hule: {
    ...
    test("役満パオ", () => {
      assert.deepEqual(
        last_log(
          convlog(
            xml +
              '<AGARI ba="1,0" hai="8,11,42,46,48" m="51785,50250,48650" machi="11" ten="40,32000,5" yakuman="39" doraHai="22" who="0" fromWho="2" paoWho="1" sc="404,323,125,-163,155,-160,316,0" />'
          )
        ),
        {
          hule: {
    ...
    test("途中流局(九種九牌)", () => {
      assert.deepEqual(
        last_log(
          convlog(
            xml +
              '<RYUUKYOKU type="yao9" ba="2,0" sc="252,0,194,0,415,0,139,0" hai0="2,5,16,17,19,22,37,70,74,115,118,126,131,134" />'
          )
        ),
        {
          pingju: {
    ...
    test("途中流局(四風連打)", () => {
      assert.deepEqual(
        last_log(
          convlog(
            xml +
              '<RYUUKYOKU type="kaze4" ba="0,0" sc="171,0,477,0,171,0,181,0" />'
          )
        ),
        {
          pingju: {
    ...
    test("途中流局(四家立直)", () => {
      assert.deepEqual(
        last_log(
          convlog(
            xml +
              '<RYUUKYOKU type="reach4" ba="2,4" sc="113,0,185,0,467,0,195,0" hai0="2,7,10,19,21,27,78,81,83,84,89,105,106" hai1="3,8,20,26,28,57,61,65,93,95,112,113,114" hai2="4,5,9,12,23,25,31,52,56,63,72,73,74" hai3="11,13,18,41,47,51,55,58,62,76,77,86,90" />'
          )
        ),
        {
          pingju: {
    ...
    test("途中流局(四槓散了)", () => {
      assert.deepEqual(
        last_log(
          convlog(
            xml +
              '<RYUUKYOKU type="kan4" ba="0,0" sc="235,0,232,0,437,0,96,0" />'
          )
        ),
        {
          pingju: {
  ...
  suite("owari", () => {
    ...
    test("順位", () => assert.deepEqual(convlog(xml).rank, [1, 4, 3, 2]));
    test("順位(同点)", () => {
      let xml =
        '<GO type="9"/>' +
        '<TAIKYOKU oya="2"/>' +
  ...
  suite("三麻", () => {
    suite("UN", () => {
      let xml =
        '<UN n0="%E5%B0%B1%E6%B4%BB%E7%94%9F%40%E5%B7%9D%E6%9D%91%E8%BB%8D%E5%9B%A3" n1="%41%53%41%50%49%4E" n2="%E2%93%85%E5%A0%80%E5%86%85%E6%AD%A3%E4%BA%BA" n3="" dan="20,20,0,0" rate="2325.68,2260.51,1500.00,1500.00" sx="M,F,M,C"/>';
      test("4人目", () => {
    ...
    suite("N", () => {
      ...
      test("北抜き", () => {
        assert.deepEqual(last_log(convlog(xml + '<N who="0" m="31008"/>')), {
          bei: { l: 2 },
        });
      });
    });
